...
class MaterialController extends HomeController
{
    function _initialize()
    {
        ...
        $res ['class'] = ($act == 'material_lists' || $act == 'add_material') ? 'current' : '';
        $nav [] = $res;
    ...
    function material_lists()
    {
        ...
        $field = 'id,title,cover_id,media_id,intro,group_id';
    ...
    function syc_news_to_wechat()
    {
        ...
        if (!isset($access_token)) {
            $this->error('无法获取access_token, 请检查公众号设置及网络是否有异常！');
        }
        ...
        if (empty ($list)) {
            $this->success('上传图文素材完成', U('material_lists'));
            exit ();
        }
// 		dump($list);
        foreach ($list as $vo) {
        ...
        $map2 ['id'] = array('not in', $ids);
        $map2 ['group_id'] = array('in', $gids);
        ...
        $url = 'https://api.weixin.qq.com/cgi-bin/material/add_news?access_token=' . $access_token;    // 新增永久图文素材
// 		dump($url);
    ...
    {
        $url = 'https://api.weixin.qq.com/cgi-bin/material/get_material?access_token=' . get_access_token(); // 获取永久素材
    ...
    function syc_news_from_wechat()
    {
        ...
        $url = 'https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token=' . $access_token;   // 分类获取永久素材列表
        $param ['type'] = 'news';
        ...
        if (empty ($list ['item'])) {
            $this->success('下载图文素材完成', U('material_lists'));
            exit ();
        }
        $map ['media_id'] = array(
            'in',
    ...
    function _thumb_media_id($cover_id)
    {
        ...
        $url = 'https://api.weixin.qq.com/cgi-bin/material/add_material?access_token=' . get_access_token();    // 新增其他类型永久素材
    ...
    function picture_lists()
    {
        ...
        $map ['manager_id'] = $this->mid;
        $map ['token'] = get_token();
        $list = M('material_image')->where($map)->field('id,cover_url,media_id')->order('id desc')->selectPage(39);
        $this->assign($list);
...
// 	    $this->assign ( 'normal_tips', '温馨提示：图片大小不超过5M,    格式: bmp, png, jpeg, jpg, gif' );
        ...
        $map['is_use'] = 1;
        $map ['token'] = get_token();
        $list = M('material_image')->where($map)->field('id,cover_url,cover_id,media_id')->order('id desc')->selectPage(39);
        $this->assign($list);
        $this->display();
    }
...
// 	    $this->assign ( 'normal_tips', '温馨提示：图片大小不超过5M,    格式: bmp, png, jpeg, jpg, gif' );
    ...
    function syc_image_to_wechat()
    {
        ...
        if (empty ($list)) {
            $this->success('上传图片素材完成', U('picture_lists'));
            exit ();
        }
        foreach ($list as $vo) {
    ...
    function syc_image_from_wechat()
    {
        ...
        if (empty ($list ['item'])) {
            $this->success('下载图片素材完成', U('picture_lists'));
            exit ();
        }

        $map ['media_id'] = array('in', getSubByKey($list ['item'], 'media_id'));
        $map ['manager_id'] = $this->mid;
        $map ['token'] = get_token();
    ...
    // 一键清空微信[图片]库
    function clear_image_from_wechat() {
        $access_token = get_access_token();
        if (!isset($access_token)) {
            $this->error('无法获取access_token, 请检查公众号设置及网络是否有异常！');
        }
        ...
        if (empty ($list ['item'])) {
            $this->success('微信端没有图片素材', U('picture_lists'));
            exit ();
        }
        if (is_array($list)) {
            $del_success = true;
            ...
            foreach ($list ['item'] as $item) {
                $del_param['media_id'] = $item ['media_id'];
                $res = post_data($del_url, $del_param);
                if (isset($res) && $list ['errcode'] != 0) {
                    $del_success = false;
                    $this->error(error_msg($list));
                }
                ...
                if ($del_success) {
                    ...
                    M('material_image')->where($map)->delete();
                }
            }
            if ($del_success) {
                $this->success('微信图片素材库被清空', U('picture_lists'));
                exit ();
            }
        }
    }
    ...
    {
        $this->assign('normal_tips', '温馨提示：语音大小不超过5M，支持mp3，wma，wav，amr格式，长度不超过60秒');
...
// 	    $this->assign ( 'normal_tips', '温馨提示：语音大小不超过5M，长度不超过60秒，支持mp3/wma/wav/amr格式' );
    ...
    function voice_add()
    {
        ...
        if (IS_POST) {
            ...
            $data = $Model->create();
            if (isset($data) && $id = $Model->add($data)) {
    ...
    function syc_voice_to_wechat()
    {
        ...
        if (empty ($list)) {
            $this->success('上传语音素材完成', U('voice_lists'));
            exit ();
        }
        foreach ($list as $vo) {
            ...
            if ($mediaId) {
                $save ['media_id'] = $mediaId;
                M('material_file')->where(array('id' => $vo ['id']))->save($save);
            }
        }
    ...
    {
        $this->assign('normal_tips', '温馨提示：视频不能超过20M，支持mp4，3gp，avi，rmvb，rm，amv，wmv，mpeg，flv格式，超过20M的视频可至腾讯视频上传后添加');
        $map['is_use'] = 1;
        $map ['manager_id'] = $this->mid;
        $map ['token'] = get_token();
        $map ['type'] = 2;
...
// 	    $this->assign ( 'normal_tips', '温馨提示：视频不能超过20M，支持大部分主流视频格式，超过20M的视频可至腾讯视频上传后添加' );
    ...
    function syc_video_to_wechat()
    {
        ...
        if (empty ($list)) {
            $this->success('上传视频素材完成', U('video_lists'));
            exit ();
        }
        foreach ($list as $vo) {
...
// 	        $this->_video_download(0, $info['down_url']);
    ...
    function syc_file_from_wechat()
    {
        ...
        if (empty ($list ['item'])) {
            if ($type == 1) {
                $this->success('下载语音素材完成', U('voice_lists'));
                exit ();
            } else {
                $this->success('下载视频素材完成', U('video_lists'));
                exit ();
            }
        }
        $map ['media_id'] = array('in', getSubByKey($list ['item'], 'media_id'));
        ...
        $url = U('syc_file_from_wechat', array('offset' => $param ['offset'] + $list ['item_count'], 'type' => $type));
        $this->success('下载微信素材中，请勿关闭', $url);
    }
