...
class MessageController extends HomeController {
	...
	public function add() {
		...
		if (IS_POST) {
			...
			$this->assign ( 'normal_tips', '温馨提示：目前微信仅开放认证公众号的群发消息权限，未认证公众号无法使用此功能' );
	...
	function preview()
	{
		...
		if ($count == 0) {
			$this->success ( '发送预览信息成功' );
		} else {
			$this->error ( '有 ' . $count . ' 条预览消息 发送失败' );
		}
	}
		...
		if (IS_POST) {
			...
			foreach ( $openidArr as $k => $v ) {
				if ($data ['msgType'] == 'text') {
					...
					$result = D ( 'Common/Custom' )->replyText ( $k, $content );
	...
	function _sucai_media_info()
	{
		...
		if ($type == 'text') {
			...
			$image_id = I ( 'image_material' );
			if (empty ($image_id)) {
				$this->error('请选择要发送的图片');
			}

			$file = M ( 'material_image' )->find ( $image_id );
			if ($file ['media_id']) {
				$res ['media_id'] = $file ['media_id'];
			} else {
				$res ['media_id'] = D ( 'Common/Custom' )->get_image_media_id ( $file ['cover_id'] );
			}
			$res ['msgtype'] = 'image';
		} else if ($type == 'appmsg') {
			...
			$file = M ( 'material_file' )->find ( $voice_id );
			if ($file ['media_id']) {
				$res ['media_id'] = $file ['media_id'];
			} else {
			...
			$file = M ( 'material_file' )->find ( $video_id );
			if ($file ['media_id']) {
	...
	function custom_sendall_lists() {
		...
		$dao = D ( 'Common/User' );
		foreach ( $list ['list_data'] as &$v ) {
			$countData = M ( 'custom_sendall' )->where ( array ('diff' => $v ['diff'], 'is_send' => 1) )->order ( 'id desc' )->group ( 'diff' )->field ( 'count(uid) count,diff' )->select ();
			$v ['chenggong'] = $countData [0] ['count'];
			$countData = M ( 'custom_sendall' )->where ( array ('diff' => $v ['diff'], 'is_send' => 0) )->order ( 'id desc' )->group ( 'diff' )->field ( 'count(uid) count,diff' )->select ();
			$v ['shibai'] = isset($countData [0] ['count']) ? $countData [0] ['count'] : 0;
			
			if ($v ['send_type'] == 0 && $v ['group_id'] == 0) {
				$v ['send_tip'] = '所有用户';
			} else if ($v ['send_type'] == 0 && $v ['group_id'] != 0) {
				$v ['send_tip'] = M ( 'auth_group' )->where ( array ('id' => $v ['group_id']) )->getField ( 'title' );
			} else {
				$v ['send_tip'] = '指定用户';
			}
			if ($v ['msgType'] == 'text') {
				$v ['Content'] = '<b>[ 文字 ]</b> <br/><br/>' . $v ['content'];
				...
				foreach ( $appMsgData as $vo ) {
					...
					if (empty ( $vo ['url'] )) {
						...
						// 文章内容
						$v ['Content'] = '<b>[ 图文 ]</b>  <br/><br/><a href="' . $art_url . '" >' . $vo ['title'] . '</a>';
					} else {
						$art_url = $vo ['url'];
						$v ['Content'] = '<b>[ 图文 ]</b>  <br/><br/><a href="' . $art_url . '" >' . $vo ['title'] . '</a>';
					}
				...
				$v ['Content'] = '<b>[ 语音 ]</b>  <br/><br/>' . $videoMaterial ['title'];
			} else if ($v ['msgType'] == 'voice') {
				$voiceMaterial = M ( 'material_file' )->find ( $vo ['voice_id'] );
				$v ['Content'] = '<b>[ 视频 ]</b>  <br/><br/>' . $voiceMaterial ['title'];
			} else if ($v ['msgType'] == 'image') {
				$v ['Content'] = '<b>[ 图片 ]</b> <br/><br/>' . '<img src="' . get_cover_url ( $v ['image_id'] ) . '" width="40px"/>';
		...
		foreach ( $list ['list_data'] as &$v ) {
			...
			if ($v ['msgtype'] == 'text') {
				...
				foreach ( $appMsgData as $vo ) {
					// $art ['description'] = $vo ['intro'];
					if (empty ( $vo ['url'] )) {
						$art_url = U ( 'Material/news_detail', array ('id' => $vo ['id']) );
						// 文章内容
						$v ['Content'] = '<b>[ 图文 ]</b> <br/><br/><a href="' . $art_url . '" >' . $vo ['title'] . '</a>';
					} else {
						$art_url = $vo ['url'];
				...
				$v ['Content'] = '<b>[ 图片 ]</b> <br/><br/>' . '<br/><img src="' . get_cover_url ( $v ['image_id'] ) . '" width="50px"/>';
				$v ['image_url'] = get_cover_url ( $v ['image_id'] );
			}
		}
		$url = U ( 'sendall_lists' );
