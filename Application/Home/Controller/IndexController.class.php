...
class IndexController extends HomeController
{
    ...
    public function main()
    {
        ...
        foreach ($data as $k => &$vo) {
            $name = $vo ['name'];
            if (isset($token_status [$name]) && $token_status [$name] === '-1') {
                unset ($data [$k]);
                // $vo ['status_title'] = '无权限';
                // $vo ['action'] = '';
                // $vo ['color'] = '#CCC';
                // $vo ['status'] = 0;
            } elseif (isset($token_status [$name]) && $token_status [$name] === 0) {
// 				$vo ['status_title'] = '已停用';
// 				$vo ['action'] = '启用';
// 				$vo ['color'] = '#CCC';
// 				$vo ['status'] = 0;
...
// 				$vo ['status'] = 1;
    ...
    public function addons_list()
    {
        ...
        $time = strtotime(date('Y-m-d'));
        $map ['u.reg_time'] = array('gt', $time);
        $count ['today'] = M()->table($px . 'public_follow as f')->join($px . 'user as u ON f.uid=u.uid')->where($map)->count();
        $map ['u.reg_time'] = array('gt', $time - 86400);
        $count ['yestoday'] = M()->table($px . 'public_follow as f')->join($px . 'user as u ON f.uid=u.uid')->where($map)->count();
        $count ['yestoday'] = $count ['yestoday'] - $count ['today'];
        $this->assign('count', $count);
