...
class WeixinController extends HomeController
{
	...
	private function reply($data, $weixin)
	{
		 ...
		 * click : 自定义菜单事件
		 */
		if ($msgType == 'event' || $msgType == 'location') {
			if ( $msgType == 'location') {
...
//					\Think\Log::write("model: \r\n" . var_export($model, true) . "\r\n\r\n", \Think\Log::INFO);
//					\Think\Log::write("event: \r\n" . var_export($event, true) . "\r\n\r\n", \Think\Log::INFO);
//					\Think\Log::write("data: \r\n" . var_export($data, true) . "\r\n\r\n", \Think\Log::INFO);
					$res = $model->$event ( $data );
//					\Think\Log::write("res: \r\n" . var_export($res, true) . "\r\n\r\n", \Think\Log::INFO);
					if ($res) {
						return true;
					}
...
//			\Think\Log::write("keyword_cache: \r\n" . var_export($keyword_cache, true) . "\r\n\r\n", \Think\Log::INFO);
...
//		\Think\Log::write("addons: \r\n" . var_export($addons, true) . "\r\n\r\n", \Think\Log::INFO);
//		\Think\Log::write("key: \r\n" . var_export($key, true) . "\r\n\r\n", \Think\Log::INFO);
		...
		$like ['token'] = array ('in', array('0', $this->token));
		...
		if (! isset($addons [$key])) {
			...
			if (empty($info['content']) && $info['group_id'] == 0 && $info['image_id'] == 0 && $info['image_material'] == 0 && $info['voice_id'] == 0 && $info['video_id'] == 0) {
				exit();
...
//		\Think\Log::write("$key: \r\n" . var_export($key, true) . "\r\n\r\n", \Think\Log::INFO);
	...
	private function init_follow($data)
	{
		...
		if (isset($data['Event']) && $data['Event'] == 'LOCATION') {
			exit();
		}
		...
		$uid = D ( 'Common/Follow' )->init_follow ( $data ['FromUserName'], $data ['ToUserName'] );
		if ($uid < 0) {
			// 初始化用户失败
			replyText("无法获取微信账号信息！");
			exit();
		}
