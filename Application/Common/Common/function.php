...
function get_access_token_by_apppid($appid, $secret) {
	...
	if (!empty($contents)) {
		$tempArr = json_decode ($contents , true );
		if (!empty($tempArr) && @array_key_exists ( 'access_token', $tempArr )) {
			S ( $key, $tempArr ['access_token'], $tempArr ['expires_in'] - 60 );
//			$cache = Think\Cache::getInstance('file', array('expire'=>$tempArr ['expires_in'] - 60)); //定义
//			$cache->set($key, $tempArr ['access_token']);
...
function isMobile() {
	...
	$agent = isset($_SERVER ['HTTP_USER_AGENT']) ? $_SERVER ['HTTP_USER_AGENT'] : null;
	// note 获取手机浏览器
	if (preg_match ( "/$mobilebrowser_list/i", $agent, $mobile )) {
		return true;
	} else {
		if (preg_match ( '/(mozilla|chrome|safari|opera|m3gate|winwap|openwave)/i', $agent )) {
			return false;
		} else {
...
function isAndroid() {
	$agent = isset($_SERVER ['HTTP_USER_AGENT']) ? $_SERVER ['HTTP_USER_AGENT'] : null;
	return strpos ( $agent, 'Android' ) !== false;
}
...
function getAddonConfig($name, $token = '') {
	...
	if (! $config) {
		$file_path = ONETHINK_ADDON_PATH . $name . '/config.php';
		if (file_exists($file_path)) {
			$temp_arr = include_once $file_path;
