...
//      	array('truename','require',-13),//验证联系人
	...
	public function autoLogin($user) {
		...
		$auth = array (
				'uid' => $user ['uid'],
				'username' => isset($user ['nickname']) ? $user ['nickname'] : '',
				'last_login_time' => isset($user ['last_login_time']) ? $user ['last_login_time'] : ''
		);
		session ( 'manager_id', $user ['uid'] );
		session ( 'mid', $user ['uid'] );
		session ( 'user_auth', $auth );
		session ( 'user_auth_sign', data_auth_sign ( $auth ) );
		session ( 'DEFAULT_THEME', isset($user ['temp']) ? $user ['temp'] : '');
	}
	...
	function updatePublicCount($uid, $public_count) {
