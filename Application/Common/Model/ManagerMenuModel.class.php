	...
	private function _get_manager_menu($uid, $addonList) {
		...
		foreach ( $menus as $m ) {
			...
			if ($m ['url_type'] == 0) {
				...
				if (isset($res ['core_side_menu']) && $res ['core_side_menu'] [$m ['id']] [0] ['id']) {
	...
	function get($uid) {
		...
		if (isset($_SERVER ['HTTP_REFERER']) && strpos ( $_SERVER ['HTTP_REFERER'], 'uploadify.swf' ) === false) {
			if (isset ( $_GET ['mdm'] )) {
				...
				$menus['current_addon'] = $current_addon;
				$menus['current_url'] = $current_url;

//				\Think\Log::write("current_addon: \r\n" . var_export($current_addon, true) . "\r\n\r\n", \Think\Log::INFO);
//				\Think\Log::write("current_url: \r\n" . var_export($current_url, true) . "\r\n\r\n", \Think\Log::INFO);

				if (isset($menus ['default_data']) && is_array($menus ['default_data'])) {
		...
		if (isset($menus ['core_side_menu']) && is_array($menus ['core_side_menu'])) {
		...
		$menus ['core_side_menu'] = $index_2 == 'home/publiclink/*' ? '' : (isset($menus ['core_side_menu']) ? $menus ['core_side_menu'] [$default ['top']] : '');
		return $menus;
	}
}
?>
