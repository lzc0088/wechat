...
class AuthManagerController extends AdminController {
	...
	function update_public_count($uid, $public_count) {
		if (empty($public_count) || $public_count < 1) {
			$this->error ( '请输入合理的公众号数量！' );
		}
		/* 调用接口更新用户最大创建公众号数量 */
		$res = D ( 'Common/User' )->updatePublicCount ( $uid, $public_count );
		if (0 < $res) { // 成功
			$this->success ( '更新成功！', U ( 'Admin/User/index' ));
		} else { // 失败，显示错误信息
			$this->error ( '更新失败或者无需更新！' );
		}
	}
}
