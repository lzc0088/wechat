...
abstract class Controller {
	...
	private function initUser() {
		...
		// 当前访问对象的uid
		$GLOBALS ['uid'] = $this->uid = intval ((isset($_REQUEST ['uid']) && $_REQUEST ['uid'] == 0) ? $this->mid : (isset($_REQUEST ['uid']) ? $_REQUEST ['uid'] : $this->mid) );
	...
	public function _search_map($model, $fields) {
		...
		$keyArr = explode ( ':', $key );
		$key = $keyArr [0];
		$placeholder = isset ( $keyArr [1] ) ? $keyArr [1] : '请输入关键字搜索';
