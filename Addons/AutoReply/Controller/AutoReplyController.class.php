        ...
        if (IS_POST) {
            ...
            if ($Model->create()) {
                ...
                if (I('id')) {        // 更新
                    ...
                    if ($result || $result === 0) {
                        // 更新keyword表数据
                        $keyword = M('keyword')->where(array('addon' => $model ['addon'], 'aim_id' => I('id')))->find();
                        if (isset($keyword)) {
                            $keyword['keyword'] = $data['keyword'];
                            $keyword_res = D ( 'Common/Keyword' )->upateByAddon ($model ['addon'], I('id'), $keyword);
                        }
                        $this->success('更新' . $model ['title'] . '成功！', $url);
                    }
                } else {            // 新增
                    $id = $Model->add($data);
                    $this->_saveKeyword($model, $id);   // 添加keyword表数据
