...
class CustomMenuTypeController extends BaseController
{
    ...
    {
        $this->assign('sub_nav', null);

        $normal_tips = '微信端菜单管理，注意：<br/>
		   &nbsp;&nbsp;&nbsp;1. 点击删除微信端所有菜单按钮，将会删除微信端默认菜单和所有个性化菜单<br/>
		   &nbsp;&nbsp;&nbsp;2. 点击个性菜单对应删除连接，则会删除相应微信端个性化菜单<br/>
...
//        }
    ...
    public function del_menu()
    {
        ...
        if ($res ['errcode'] == 0) {
            ...
            if ($result) {
                $this->success('全部菜单删除成功,平台数据更新成功！');
            } else {
                $this->error('全部菜单删除成功,平台数据更新失败！');
            }
        } else {
            $this->error(error_msg($res));
        }
    ...
    public function del()
    {
        ...
        $mapa = array('menu_id' => array('in', $ids));
