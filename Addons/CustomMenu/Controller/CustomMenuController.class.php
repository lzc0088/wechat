...
class CustomMenuController extends BaseController {
	...
	function send_menu() {
		...
		if ($res ['errcode'] == 0) {
            $result = D ( 'CustomMenuType' )->set_menu_status(1,$menu_id,null);
            if ($result) {
...
// 			}
			...
			$this->assign ( 'normal_tips', '为了方便移植，可以用以下特殊字符代替常用地址参数：<br>
				[website]：' . SITE_URL . '，[publicid]：' . get_token_appinfo ( '', 'id' ) . '，[token]：' . get_token () . '
				<br>微网站：[website]/?s=/addon/WeiSite/WeiSite/index/publicid/[publicid]
				<br>宣传页：[website]/?s=/home/Index/leaflets/token/[token]' );
...
// 			}
	...
	public function del() {
		...
		if (I ( 'id' )) {
			// delete child
			M('custom_menu')->where(array('pid' => I ( 'id' )))->delete();
		}
